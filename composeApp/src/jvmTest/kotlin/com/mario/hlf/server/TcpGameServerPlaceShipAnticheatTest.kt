...
class TcpGameServerPlaceShipAnticheatTest {

    @Test
    fun `PLACE_SHIP with wrong player id returns FORBIDDEN`() {
        val config = ServerConfig(host = "127.0.0.1", port = 5688, maxClients = 10)
        ...
        try {
            s1 = Socket("127.0.0.1", 5688)
            s2 = Socket("127.0.0.1", 5688)
            ...
            // P2 intenta colocar barco diciendo que es P1
            send(
                s2, Envelope(
                    v = 1,
                    requestId = "place-wrong-player",
                    gameId = gameId1,
                    payload = PlaceShip(
                        player = PlayerId.P1, // MAL: sesión es P2
                        gameId = gameId1,
                        row = 0,
                        col = 0,
